...
class GameInstance {
	...
	private mSecElapsed = 0;
	/* used to define a condition based on time elapsed */
	private runStateStartWaitingTime = 0;
	private runStateStartBeforeGameTime = 0;
	...
	private globalFreezeStartTimer = 0;
	private forceGlobalFreeze = true;
	...
	private playerOneLastActionProcessed = -1;
	private playerTwo: PlayerState = cloneDeep(samplePlayer);
	private playerTwoLastActionProcessed = -1;
	...
	constructor(
		...
		if (givenGameOptions) Object.assign(this.gameOptions, givenGameOptions);
	...
	private async run() {
		...
		while (this.runState !== RUNSTATE.ENDED) {
			...
			this.powersHandler();
			if (!this.ballState.flags.freezed) this.ballHandler(delta);
			this.globalFreezeHandler();

			this.responseRefresh();
			this.wsServer
				.to(this.wsRoom)
				.emit('stateUpdate', this.responseState);
	...
	private runStateHandler() {
		if (
			...
			if (
				!this.runStateStartWaitingTime &&
				(!this.playerOne.connected || !this.playerTwo.connected)
			) {
				this.runStateStartBeforeGameTime = 0;
				this.runStateStartWaitingTime = this.mSecElapsed;
			}
			if (
				!this.runStateStartBeforeGameTime &&
				this.playerOne.connected &&
				this.playerTwo.connected
			) {
			...
			if (
				...
				this.mSecElapsed -
					this.runStateStartWaitingTime -
					(this.runState === RUNSTATE.WAITING ? 120 : 60) * 1000 >
					-50
			) {
				this.runStateStartBeforeGameTime = 0;
				this.runStateStartWaitingTime = 0;
			...
			if (
				this.runStateStartBeforeGameTime &&
				this.mSecElapsed - this.runStateStartBeforeGameTime - 5 * 1000 >
					-50
			) {
				this.runStateStartBeforeGameTime = 0;
				this.runStateStartWaitingTime = 0;
			...
			if (!this.runStateStartWaitingTime)
				...
				this.runStateStartWaitingTime &&
				this.mSecElapsed - this.runStateStartWaitingTime - 1000 > -50
			)
				this.runState = RUNSTATE.ENDED;
		}
	}
	...
	private responseRefresh() {
		...
		this.responseState.playerOneLastActionProcessed =
			this.playerOneLastActionProcessed;
		this.responseState.playerTwoLastActionProcessed =
			this.playerTwoLastActionProcessed;
	}

	// incoming position changes
	private movementHandler(gameAction: GameAction, isPlayerOne: boolean) {
		const playerState: PlayerState = isPlayerOne
			? this.playerOne
			: this.playerTwo;
		if (!playerState.flags.stuned) {
			playerState.pos.y = gameAction.data.y as number;
			if (isPlayerOne) this.playerOneLastActionProcessed = gameAction.id;
			else this.playerTwoLastActionProcessed = gameAction.id;
		}
	}

	private scoreRegister() {
		const players: Array<PlayerState> = [this.playerOne, this.playerTwo];
		const winner: number = this.ballState.pos.x < 50 ? 1 : 0; // round winner index
		...
		if (players[winner].score >= 6) {
			// a player won
			this.forceGlobalFreeze = true;
			this.runState = RUNSTATE.AFTER_GAME;

			this.gameService.saveMatchResult(
				players[winner],
				players[Math.abs(winner - 1)]
			);
		}
	...
	private globalFreezeSetter(options?: {
		resetPlayerPos?: boolean;
		resetBallPos?: boolean;
	}) {
		const players: Array<PlayerState> = [this.playerOne, this.playerTwo];
		for (const player of players) {
			if (options?.resetPlayerPos) player.pos.y = 50;
		...
		if (options?.resetBallPos) {
			this.ballState.directionVector.x =
				this.ballState.pos.x < 50 ? -1 : 1;
	...
	private globalFreezeHandler() {
		if (this.globalFreezeStartTimer) {
			const msDifference: number =
				this.mSecElapsed - this.globalFreezeStartTimer - 3000;
	...
	private chargingHandler(gameAction: GameAction, isPlayerOne: boolean) {
		const playerState: PlayerState = isPlayerOne
			...
			: this.playerTwo;
		if (
			!playerState.flags.stuned &&
			this.gameOptions.gameType === 'extended' &&
			!playerState.capacityTimeTrigger
		) {
	...
	private ballHandler(delta: number) {
		const ballSpeed: number =
			this.ballState.speedPPS * (this.ballState.flags.smash ? 2 : 1);
		this.ballState.pos.x +=
			this.ballState.directionVector.x *
			((delta * (ballSpeed / 10)) / 100);
		this.ballState.pos.y +=
			this.ballState.directionVector.y *
			((delta * (ballSpeed / 10)) / 100);
	...
	private ballCollisionPlayerChecker(
		gameAction: GameAction,
		isPlayerOne: boolean
	) {
		let differencePlayerY = 0;
		const differenceBall: number[] = [];
		const differenceBallEaseRange: number = this.ballState.flags.smash
			? 10
			: 10;
		const currentPlayer: PlayerState = isPlayerOne
			? this.playerOne
			: this.playerTwo;
		if (
			isPlayerOne
				? (gameAction.data.ballPos?.x as number) <= 25
				: (gameAction.data.ballPos?.x as number) >= 75
		) {
			differencePlayerY =
				currentPlayer.pos.y - (gameAction.data.y as number);
			differenceBall[0] =
				this.ballState.pos.x - (gameAction.data.ballPos?.x as number);
			differenceBall[1] =
				this.ballState.pos.y - (gameAction.data.ballPos?.y as number);
			if (
				differencePlayerY < -differenceBallEaseRange ||
				differencePlayerY > differenceBallEaseRange ||
				differenceBall[0] < -differenceBallEaseRange ||
				differenceBall[0] > differenceBallEaseRange ||
				differenceBall[1] < -differenceBallEaseRange ||
				differenceBall[1] > differenceBallEaseRange
			)
				return;
			this.ballState.directionVector.x = isPlayerOne ? 1 : -1;
			// angle computation
			const topOfRacket: number =
				(gameAction.data.y as number) -
				100 / this.gameOptions.racketSize / 2;
			let ballPercentageRacket: number = Math.floor(
				((gameAction.data.ballPos?.y as number) - topOfRacket) /
					(100 / this.gameOptions.racketSize / 100)
			);
			if (ballPercentageRacket < 10) ballPercentageRacket = 10;
			if (ballPercentageRacket > 90) ballPercentageRacket = 90;
			let newAngle = -10;
			for (
				let step = 10;
				!(
					ballPercentageRacket >= step &&
					ballPercentageRacket <= step + 9
				) && newAngle < 6;
				step += 10
			)
				newAngle += 2;
			...
			// smash signal
			if (
				currentPlayer.capacityLoaderPercentage >= 98 &&
				currentPlayer.stockedCapacity === PLAYER_CAPACITY.SMASH
			) {
				this.ballState.flags.freezed = true;
	...
	private movementChecker() {
		const hundredMsMovementAllowed: number =
			(this.gameOptions.yDistPPS / 100) * 15; // 5% ease range
		const percentageHalfRacketSize: number =
			100 / this.gameOptions.racketSize / 2;
		const players: Array<PlayerState> = [this.playerOne, this.playerTwo];
		let i = 0;
		for (const player of players) {
			if (!player.flags.stuned) {
				const responsePlayer: PlayerState = !i
					? this.responseState.playerOne
					: this.responseState.playerTwo;
				if (player.pos.y > responsePlayer.pos.y) {
					// goes bottom
					if (
						player.pos.y - responsePlayer.pos.y >
						hundredMsMovementAllowed
					)
						player.pos.y =
							responsePlayer.pos.y + hundredMsMovementAllowed;
					...
					if (
						responsePlayer.pos.y - player.pos.y >
						hundredMsMovementAllowed
					)
						player.pos.y =
							responsePlayer.pos.y - hundredMsMovementAllowed;
			...
			for (const player of players) {
				if (player.flags.capacityCharging) {
					if (player.capacityLoaderPercentage < 100) {
						player.capacityLoaderPercentage +=
							(delta * (this.gameOptions.capChargingPPS / 10)) /
							100;
						if (player.capacityLoaderPercentage > 100)
							player.capacityLoaderPercentage = 100;
					}
				} else if (
					this.ballState.flags.smash &&
					this.ballState.flags.freezed &&
					player.capacityLoaderPercentage >= 98
				) {
					player.capacityLoaderPercentage = 100;
				} else {
	...
	private powersHandler() {
		...
		for (const player of players) {
			if (player.capacityTimeTrigger > 0) {
				const msDifference: number =
					this.mSecElapsed -
					player.capacityTimeTrigger -
					playerCapacityDelay[player.stockedCapacity];
				if (msDifference > -50) {
					if (player.stockedCapacity === PLAYER_CAPACITY.STUNNING) {
						if (player === this.playerOne)
							this.playerTwo.flags.stuned = false;
						else this.playerOne.flags.stuned = false;
					} else if (
						player.stockedCapacity === PLAYER_CAPACITY.SMASH
					) {
				...
				if (player.stockedCapacity === PLAYER_CAPACITY.STUNNING) {
					...
					else this.playerOne.flags.stuned = true;
				}
			}
		}
	}
		...
		if (
			...
			if (!this.ballState.directionVector.y) {
				...
				if (this.ballState.directionVector.y < 0)
					points =
						(10 / (this.ballState.directionVector.y * -10 - 10)) *
						-2;
				else
					points =
						(10 / (this.ballState.directionVector.y * 10 - 10)) *
			...
			if (this.ballState.flags.smash) points *= 2;
			currentPlayer.capacityUnlockerPercentage += points;
			if (currentPlayer.capacityUnlockerPercentage > 100)
				currentPlayer.capacityUnlockerPercentage = 100;
			if (currentPlayer.capacityUnlockerPercentage === 100)
				currentPlayer.stockedCapacity =
					Math.random() > 0.5
						? PLAYER_CAPACITY.STUNNING
						: PLAYER_CAPACITY.SMASH;
		}
	}
	...
	injectGameAction(gameAction: GameAction, playerId: number) {
		if (
			...
			if (
				gameAction.keyPressed === GA_KEY.UP ||
				gameAction.keyPressed === GA_KEY.DOWN
			)
			...
			if (gameAction.keyPressed === GA_KEY.SPACE)
				this.chargingHandler(
					gameAction,
					playerId === this.playerOne.id
				);
			if (
				gameAction.keyPressed === GA_KEY.NONE &&
				gameAction.data.ballPos
			)
				this.ballCollisionPlayerChecker(
					gameAction,
					playerId === this.playerOne.id
				);
		}
