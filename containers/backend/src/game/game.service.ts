...
export class GameService {
	...
	async createNewGame(
		...
		if (players.length !== 2) {
			throw new Error(
				'One or both of the players are not existing users.'
			);
		}
		...
		throw new Error(
			'Game gateway is not initialized for now, please wait.'
		);
	}
	...
	checkMatch(playerId: number): { id: number; error?: string } {
		...
		const match = this.list.finished.find((match) => {
			return (
				playerId === match.playerOne.id ||
				playerId === match.playerTwo.id
			);
		...
		if (match.error) return { id: 0, error: match.error };
		return { id: match.id };
	}
	...
	matchmakingAddPlayer(player: User) {
		let i = 0;
		while (
			i < this.list.waiting.length &&
			this.list.waiting[i].user.elo > player.elo
		) {
			i++;
	...
	getMatchId(userId: number): number {
		...
		const match = this.list.finished.find((match) => {
			return (
				userId === match.playerOne.id || userId === match.playerTwo.id
			);
		}); // find the user in the list of matches created
	...
	async saveMatchResult(winner: PlayerState, loser: PlayerState) {
		...
		const players: User[] = await this.userRepo.find({
			where: [{ id: winner.id }, { id: loser.id }],
			relations: ['history'],
		});
		...
		// compute new elo
		const updatedElo = this.computeNewEloScore(
			players[0].elo,
			players[1].elo
		);
	...
	private async matchWaitingPlayers() {
		while (true) {
			...
			while (i < this.list.waiting.length) {
				...
				const actualDiff =
					this.list.waiting[i - 1].user.elo -
					this.list.waiting[i].user.elo;
				// every 15s the allowed elo gap is bigger by 250 elo
				const allowedDiff =
					250 *
					(Math.floor(
						(Date.now() - this.list.waiting[i].time) / 15000
					) +
						1);
				...
				if (actualDiff <= allowedDiff) {
					...
					let timer: number;
					try {
						last.id = await this.createNewGame([
							last.playerOne.id,
							last.playerTwo.id,
						]);
