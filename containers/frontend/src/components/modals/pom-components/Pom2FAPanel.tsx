...
const Pom2FAPanel = () : JSX.Element => {
	...
	const [ btnText, setBtnText ] = useState<number>(loginStatus.user?.twofa ? 1 : 0);
			...
			<div className="pom-2fa-content">
				<div className="pom-2fa-qrcode-container">
					...
					{!tfaStartProcedure && <button 
						style={loginStatus.user?.twofa ? spbLabel[1].style : spbLabel[0].style}
						onClick={(e) => {
							if (btnText < 2) {
								setBtnText(2);
								tfaBtnHandler(
									!loginStatus.user?.twofa, 
									setQrCode, 
									setBtnText,
