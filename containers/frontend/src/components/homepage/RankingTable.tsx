...
const RankingTable = (props: IProps) => {
	...
	const executeScrollUser = (e: any) => {
		if (scrollUserRef.current) scrollUserRef.current.scrollIntoView();
	};

	const renderBodyData = () => {
		const ladder = rankingData.data;
		...
		return (
			<>
				...
				<div className="row-buttons">
					<button className="button-top" onClick={executeScrollTop}>
						go to the top
					</button>
					<button className="button-rank" onClick={executeScrollUser}>
						show my rank
					</button>
				</div>
			</>
