...
export class OnlineStateGuard implements CanActivate {
	...
	async canActivate(context: ExecutionContext): Promise<boolean> {
		...
		if (contextUser && contextUser.isAuthenticated()) {
			...
			if (
				(contextUser.session as Session & Session2FaDTO).twofa.needed &&
				!(contextUser.session as Session & Session2FaDTO).twofa.passed
			)
				return true;
			// switch to online if the user wasn't
			if (
				!(contextUser.user as User).status ||
				(contextUser.user as User).status === 'offline'
			) {
			...
			if (
				!(contextUser.session as Session & SessionOnlineStateDTO)
					.keepUpOnlineState
			) {
				(
					contextUser.session as Session & SessionOnlineStateDTO
